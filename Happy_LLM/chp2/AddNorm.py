"""
前馈网络 (FeedForward) 与 残差归一化 (AddNorm) 组件
- FeedForward: 典型的升维 -> 激活 -> 降维结构，用于非线性特征变换
- AddNorm: 残差连接 + LayerNorm（本例采用 Pre-LN：先归一化后进入子层，再残差相加）
适用场景：Transformer Block 内的子模块封装与组合
"""
import torch
...
class FeedForward(nn.Module):
    def __init__(self, d_model, d_ff, dropout=0.1):
        super().__init__()
        # 升维（d_model -> d_ff）-> ReLU 激活 -> Dropout -> 降维（d_ff -> d_model）
...
class AddNorm(nn.Module):
    ...
    def forward(self, x, sublayer_fn):
        # 1) 先做 LayerNorm（Pre-LN）
        norm_x = self.norm(x)
        # 2) 进入子层（Attention 或 FFN），保持形状一致
        sub_output = sublayer_fn(norm_x)
        # 3) 残差连接：x + Dropout(sub_output)
