"""
Transformer 层 (TransformerLayer) 组合
- 组件：多头自注意力 (MHA) + 前馈网络 (FFN) + 两次 AddNorm（残差归一化）
- 规范：本例采用 Pre-LN（先归一化再进入子层，最后残差相加），更稳定易训练
注意：
- 依赖 MHA 的实现（from day2_solution import MultiHeadAttention），需确保同目录或可导入
"""
import torch
...
class TransformerLayer(nn.Module):
    ...
    def forward(self, x, mask=None):
        # 第一步：自注意力 + 残差归一化（输出形状与 x 保持一致）
        x = self.add_norm1(x, lambda x: self.attn(x, x, x, mask)[0])
        # 第二步：前馈网络 + 残差归一化（输出形状与 x 保持一致）
