...
Original URL: https://www.waylandz.com/llm-transformer-book/第11章-Multi-Head-Attention-多视角理解/
...
## 11.2 切分多头：把 d\_model 分成 num\_heads 份

### 11.2.1 维度切分

![切分多头](images/ch11-01-split-heads.jpg)
...
* d\_model = 512
* num\_heads = 4
* 则 d\_key = d\_model / num\_heads = 512 / 4 = 128
...
![切分方式对比](images/ch11-05-split-comparison.jpg)
...
* 用一个大的 Wq 矩阵生成完整的 Q
* 然后把 Q 沿着 d\_model 维度切成 num\_heads 份
* 这是**实际实现**的方式（更高效）
...
![完整多头计算](images/ch11-02-multihead-full.jpg)
...
![合并和Wo](images/ch11-03-concatenate-wo.jpg)
...
![输出对比](images/ch11-04-output-comparison.jpg)
...
| 矩阵 | 形状 | 参数量 |
| --- | --- | --- |
| Wq | [d\_model, d\_model] | d\_model² |
| Wk | [d\_model, d\_model] | d\_model² |
| Wv | [d\_model, d\_model] | d\_model² |
| Wo | [d\_model, d\_model] | d\_model² |

总参数量 = **4 × d\_model²**

以 GPT-2 Small 为例（d\_model = 768）：

* 每层 Attention 参数量 = 4 × 768² ≈ 236 万

### 11.7.2 常见配置

| 模型 | d\_model | num\_heads | d\_key |
| --- | --- | --- | --- |
...
假设 d\_model = 512，num\_heads = 8，d\_key = 64：
...
常见的 d\_key 通常是 64 或 128，这是一个经验性的平衡点。
...
| **num\_heads** | 头的数量 |
| **d\_key** | 每个头的维度 = d\_model / num\_heads |
| **切分** | 把 d\_model 维度分成 num\_heads 份 |
| **合并** | 把所有头的输出拼接回 d\_model |
| **Wo** | 输出投影矩阵，融合各头信息 |
...
* 说出 d\_key = d\_model / num\_heads 的关系
* 理解切分和合并的维度变化
* 知道 Wo 矩阵的作用（融合各头信息）
* 解释不同头可能学习到的不同模式
...
textMultiHead(Q,K,V)=textConcat(texthead1,...,textheadh)WO\\text{MultiHead}(Q, K, V) = \\text{Concat}(\\text{head}\_1, ..., \\text{head}\_h) W^OtextMultiHead(Q,K,V)=textConcat(texthead1​,...,textheadh​)WO

其中：

textheadi=textAttention(QWiQ,KWiK,VWiV)\\text{head}\_i = \\text{Attention}(QW\_i^Q, KW\_i^K, VW\_i^V)textheadi​=textAttention(QWiQ​,KWiK​,VWiV​)
