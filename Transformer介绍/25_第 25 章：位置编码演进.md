...
Original URL: https://www.waylandz.com/llm-transformer-book/第25章-位置编码演进-Sinusoidal到RoPE到ALiBi/
...
![位置编码技术概览](images/ch25-01-pe-overview.jpg)
...
![Sinusoidal 位置编码与 RoPE 对比](images/ch25-03-rope-rotation.jpg)
...
![RoPE 旋转位置编码](images/ch25-04-rope-2d.jpg)
...
![RoPE 实现细节](images/ch25-05-rope-complex.jpg)

真实的词向量不是 2D 的，而是 768D 或 4096D。RoPE 的处理方式是：**把高维向量拆成多个 2D 子空间，每个子空间独立旋转**。

假设 d=6（6 维向量），会被拆成 d/2=3 对：

* 第 1-2 维：用角度 theta\_1 旋转
* 第 3-4 维：用角度 theta\_2 旋转
* 第 5-6 维：用角度 theta\_d/2 旋转
...
![RoPE 公式分解](images/ch25-06-rope-formula.jpg)
...
假设位置 m 的 Query 是 q\_m，位置 n 的 Key 是 k\_n。

应用 RoPE 后：

* q\_m' = R\_m \* q\_m（旋转 m \* theta）
* k\_n' = R\_n \* k\_n（旋转 n \* theta）
...
关键来了：**R\_m^T \* R\_n = R\_(n-m)**，这是旋转矩阵的性质！
...
![RoPE 旋转矩阵与长期衰减](images/ch25-07-alibi-intro.jpg)
...
![RoPE 热力图](images/ch25-08-alibi-matrix.jpg)
...
![RoPE 螺旋类比](images/ch25-09-yarn-intro.jpg)
...
![ALiBi 线性偏置](images/ch25-11-pe-summary.jpg)
...
![YaRN NTK-aware](images/ch25-10-yarn-ntk.jpg)
