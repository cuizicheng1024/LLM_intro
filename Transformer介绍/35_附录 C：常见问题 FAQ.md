...
Original URL: https://www.waylandz.com/llm-transformer-book/附录C-常见问题FAQ/
...
常见配置：head\_dim = 64 或 128，head\_num = d\_model / head\_dim

| 模型 | d\_model | num\_heads | head\_dim |
| --- | --- | --- | --- |
| GPT-2 Small | 768 | 12 | 64 |
| LLaMA-7B | 4096 | 32 | 128 |
| GPT-3 | 12288 | 96 | 128 |

经验上，head\_dim = 64-128 效果较好。
...
* 小模型（`<1B）：batch\_size = 512-2048
* 大模型（>1B）：batch\_size = 1M-4M tokens（通过梯度累积）
...
在生成 x\_t 之前，必须先知道 x\_1 到 `x_{t-1}`。所以只能一个一个生成。
...
2. **没有重复惩罚**：增加 repetition\_penalty
3. **训练数据有重复**：数据质量问题

解决：增加 temperature，加入 repetition\_penalty (1.1-1.2)。
