...
Original URL: https://www.waylandz.com/llm-transformer-book/第15章-Transformer完整前向传播-从输入到输出/
...
![Decoder-Only架构](images/ch15-08-decoder-only.jpg)
...
![GPT架构对比](images/ch15-01-gpt-architecture-comparison.jpg)
...
![位置编码可视化](images/ch15-09-pe-visualization.png)
...
![Transformer Block](images/ch15-14-transformer-block.png)
...
![Attention Block Step](images/ch15-10-attention-block-step.png)
...
![Self-Attention Diagram](images/ch15-11-self-attn-diagram.png)
...
![QK Attention Raw](images/ch15-12-qk-attention-raw.png)
...
![QK Attention Masked](images/ch15-13-qk-attention-masked.png)
...
![完整架构参数](images/ch15-04-full-architecture-params.jpg)

这张图标注了所有参数的位置：

| 组件 | 参数公式 | 示例（d\_model=512, vocab=100256） |
| --- | --- | --- |
| Word Embedding | vocab × d\_model | 5130万 |
| Attention (×12) | 4 × d\_model² × 12 | 1258万 |
| FFN (×12) | 8 × d\_model² × 12 | 2516万 |
| Output Linear | d\_model × vocab | 5130万 |
...
![输出线性层](images/ch15-02-output-linear-wp.jpg)
...
![Wp矩阵乘法](images/ch15-03-wp-matmul.jpg)

Wp 矩阵可以理解为：**词表中每个词都有一个 d\_model 维的"特征向量"**。

输出 logits = 输入向量和每个词的特征向量的**点积（相似度）**。

![Wp解释](images/ch15-06-wp-explanation.jpg)

### 15.6.4 Softmax 得到概率

![最终概率分布](images/ch15-15-final-probabilities.png)
...
| --- | --- | --- | --- |
| d\_model | 模型维度 | 768 | 1280 |
| n\_layers | Block 层数 | 12 | 36 |
| n\_heads | 注意力头数 | 12 | 20 |
| d\_ff | FFN 隐藏层 | 3072 | 5120 |
| vocab\_size | 词表大小 | 50257 | 50257 |
...
以 GPT-2 Small 为例（d\_model=768, n\_layers=12, vocab\_size=50257）：

| 组件 | 公式 | 参数量 |
| --- | --- | --- |
| Token Embedding | vocab × d\_model | 3860万 |
| Position Embedding | max\_len × d\_model | 79万 |
| Attention (×12) | 4 × d\_model² × 12 | 2831万 |
| FFN (×12) | 2 × d\_model × d\_ff × 12 | 5662万 |
| LayerNorm (×25) | 2 × d\_model × 25 | 4万 |
...
![反向传播](images/ch15-05-backprop-update.jpg)
...
| 2 | Embedding | IDs → 向量 [seq, d\_model] |
| 3 | + Position | 加位置信息 |
| 4 | Attention | 理解词间关系 |
...
> **Transformer 的前向传播是一个优雅的流水线：输入的文字被转换成向量，经过多层 Block 的处理（每层都包含 Attention 理解上下文、FFN 提取特征），最后映射到词表得到概率分布。整个过程中，维度在 Block 中保持不变（都是 d\_model），只在最后映射时从 d\_model 变成 vocab\_size。**
