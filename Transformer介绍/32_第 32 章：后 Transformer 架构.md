...
Original URL: https://www.waylandz.com/llm-transformer-book/第32章-后Transformer架构-Mamba与混合模型/
...
h(ˊt)=Ah(t)+Bx(t)h\'(t) = Ah(t) + Bx(t)h(ˊ​t)=Ah(t)+Bx(t)
...
hk=barAhk−1+barBxkh\_k = \\bar{A}h\_{k-1} + \\bar{B}x\_khk​=barAhk−1​+barBxk​

yk=Chky\_k = Ch\_kyk​=Chk​

这里 barA\\bar{A}barA 和 barB\\bar{B}barB 是通过离散化得到的参数。

常用的离散化方法是**零阶保持（Zero-Order Hold）**：

barA=eDeltaA\\bar{A} = e^{\\Delta A}barA=eDeltaA

barB=(eDeltaA−I)A−1B\\bar{B} = (e^{\\Delta A} - I)A^{-1}BbarB=(eDeltaA−I)A−1B

其中 Delta\\DeltaDelta 是采样间隔。
...
Deltak=textsoftplus(textLinear(xk))\\Delta\_k = \\text{softplus}(\\text{Linear}(x\_k))Deltak​=textsoftplus(textLinear(xk​))

Bk=textLinear(xk)B\_k = \\text{Linear}(x\_k)Bk​=textLinear(xk​)

Ck=textLinear(xk)C\_k = \\text{Linear}(x\_k)Ck​=textLinear(xk​)

注意：

* Delta\\DeltaDelta（离散化步长）变成了输入的函数
* B（输入投影）变成了输入的函数
* C（输出投影）变成了输入的函数

**直觉理解**：

* 当 Delta\\DeltaDelta 很大时，隐状态快速更新 → "关注当前输入"
* 当 Delta\\DeltaDelta 很小时，隐状态保持稳定 → "保持历史记忆"
...
Deltak,Bk,Ck=f(xk)\\Delta\_k, B\_k, C\_k = f(x\_k)Deltak​,Bk​,Ck​=f(xk​)
